...
function DashBoard() {
  ...
  const fetchTasksByWeek = async (selectedPeriod) => {
    try {
      ...
      if (isLoggedIn) {
        backlogTasks = await getAllTaskByWeek({
          period: selectedPeriod,
          status: "BackLog",
        });
        todoTasks = await getAllTaskByWeek({
          period: selectedPeriod,
          status: "ToDo",
        });
        inProgressTasks = await getAllTaskByWeek({
          period: selectedPeriod,
          status: "InProgress",
        });
        doneTasks = await getAllTaskByWeek({
          period: selectedPeriod,
          status: "Done",
        });
      }
  ...
  return (
    <div className={styles.mainContainer}>
      ...
      <div className={styles.headingDiv}>
        ...
        <select
          className={styles.weekSelection}
          value={period}
          onChange={handlePeriodChange}
        >
          <option value="today" style={{ border: "none" }}>
            Today
          </option>
          ...
          <div className={styles.taskHead}>
            ...
            <div>
              <img
                ...
                alt="collapse"
                style={{
                  visibility: localStorage.getItem("name")
                    ? "visible"
                    : "hidden",
                  width: "33%",
                  height: "40%",
